 ...
 *   Local debug log:  UTX2 on RPB10 (the carrier's own PGD/debug line,
 *     visible through an upstream writer32mxcdc passthrough).  UART2 is
 *     free here, so the log stays active the whole time.
 *
...
void main(void)
{
	...
	TRISA = 0x0000; /* -------- ---O--OO */
	TRISB = 0x2804; /* OOI-IOOO O-OOOIOO */

	ANSELA = 0;
	ANSELB = 0;

	LATA = 0xfffd;	/* RA1: target MCLR held low until "run"; RA0 idle high */
	LATB = 0xffff;
	...
	/* UART2: local debug log on RPB10 (the carrier's own PGD/debug
	   line, visible through an upstream writer32mxcdc passthrough). */
	RPB10R = 2;		/* UTX2 */
#ifdef DEBUG_UART_SCAN
