 ...
 * so unmodified targets (e.g. flashsample.c) work as-is.
 */
...
#define LAT_PGC0 LATBbits.LATB9
#define LAT_MCLR0 LATAbits.LATA1
#define PORT_PGD0 PORTBbits.RB8
#define LAT_PGD0 LATBbits.LATB8
#define TRIS_PGD0 TRISBbits.TRISB8
...
static void passthrough_start(void)
{
	...
	RPB9R = 2;	/* UTX2 -> target URX2 (PGEC2) */
	U2RXR = 4;	/* RPB8 <- target UTX2 (PGED2) */
	TRIS_PGD0 = 1;	/* in */
...
static void writeflash(W isfinal)
{
	...
	if (!writing) {
		...
		RPB9R = 0; /* PGC: i/o */
		U2RXR = 0; /* park:RPA1 (our MCLR line) */
		writing = 1;
