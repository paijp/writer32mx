/*
 * writer32mx-wroom-c20p1305-barcodeuart - Wi-Fi ICSP flash programmer
 * https://github.com/paijp/writer32mx
 *
 * writer32mx variant that talks to the host over Wi-Fi (ESP-WROOM-02,
 * AT firmware) instead of CDC/UART, secured with ChaCha20-Poly1305
 * (c20p1305.h, from paijp/single-file-chacha20poly1305, MIT/PD).
 *
 * Pairing works like the wroomc20p1305 samples: for 10 s after boot a
 * 9600 bps serial barcode reader (wired through a weak resistor onto
 * the WROOM->PIC32 line) may deliver two scans, saved to flash:
 *   WIFI:T:WPA;S:<ssid>;P:<pass>;;
 *   C20P:K:<64 hex>;U:<full URL up to "key0c20=">;;
 * After the window the stored credentials drive the connect, then the
 * writer loop runs forever:
 *   - target debug serial (115200 8N1) is buffered and sent to the
 *     server as the request payload ("s0800=" + raw bytes)
 *   - the decrypted reply is fed to the Intel HEX parser: non-hex
 *     bytes echo to the target UART, HEX records program the target
 *     via 2-wire Enhanced ICSP (with mid-stream flush)
 *   - if both the send buffer and the last reply were empty the next
 *     server access waits 2 s (aborted early when target data
 *     arrives); otherwise it happens immediately
 *
 ...
 *   Local debug log:  UTX2 mirror on RPB10 (the classic PGD/debug line,
 *     visible through an upstream writer32mxcdc passthrough); muted once
 *     the writer loop starts because UART2 then belongs to the target.
 *
...
/* ============================================================ */
/* Local debug log (UTX2 mirror on RPB10)                       */
/*                                                              */
...
static	void	barcode_window(void)
{
	...
	while (tmout > 0) {
		...
		/* Test hook (no barcode reader): during the scan window also
		   accept barcode text on U2RX/RPB11 — the upstream debug line
		   an attached writer32mxcdc passthrough talks to. */
		if ((U2STAbits.OERR))
			U2STA = 0x1400;
		if ((U2STAbits.URXDA)) {
			v = U2RXREG;
...
void main(void)
{
	...
	TRISA = 0x0000; /* -------- ---O--OO */
	TRISB = 0x2802; /* OOI-IOOO O-OOOOIO */

	ANSELA = 0;
	ANSELB = 0;

	LATA = 0xfffd;	/* RA1: target MCLR held low until "run" */
	LATB = 0xffff;
	...
	/* UART2: local debug mirror on RPB10 during setup; routed onto the
	   target pins (RPB0/RPB1) once the writer loop starts. */
	RPB10R = 2;		/* UTX2 */
#ifdef DEBUG_UART_SCAN
	...
	/* From here UART2 belongs to the target: mute the local debug
	   mirror so WROOM chatter stays off the target's RX line. */
	lcdtp_quiet = 1;
