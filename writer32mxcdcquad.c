...
/*
 * Pin assignments (writer -> 4 targets):
 *
 *   target  PGC (-> PGEC2/URX2)  MCLR   PGD (-> PGED2/UTX2)
 *   0       RA0                  RA1    RB2
 *   1       RB4                  RB14   RA4
 *   2       RB7                  RB8    RB5
 *   3       RB15                 RB9    RB13
 *
 * All PGC pins can carry UTX1 through PPS, so the host->target serial
 * can be sent to any combination of targets at once.  PGD0/1/3 can be
 * U1RX; PGD2 (RB5) cannot, so target 2 is received through U2RX.
 *
 * Host commands (in addition to Intel HEX):
 *   :ff     reset all targets (MCLR0-3)
 *   :feXY   X (0-3): target whose serial goes to USB, and which is
 *           programmed when HEX data arrives.
 *           Y (bitmask 0-f): targets that receive the USB serial.
 *           e.g. ":fe25" = receive from target 2, send to targets 0 and 2.
 */
...
/*
 * Per-target pin table.  lat/tris/port point at LATx/TRISx/PORTx;
 * [1] is the CLR register and [2] the SET register.
 */
struct target_pins {
	volatile UW *pgclat;
	UW pgcbit;
	volatile UW *pgcrp;	/* RPxnR of the PGC pin */
	volatile UW *mclrlat;
	UW mclrbit;
	volatile UW *pgdlat;
	volatile UW *pgdtris;
	volatile UW *pgdport;
	UW pgdbit;
	W rxsel;	/* U1RXR value, or -1 when received by U2RX */
};

#define NTARGET 4
static const struct target_pins targets[NTARGET] = {
	{ &LATA, 1 << 0, &RPA0R, &LATA, 1 << 1,
	  &LATB, &TRISB, &PORTB, 1 << 2, 4 },
	{ &LATB, 1 << 4, &RPB4R, &LATB, 1 << 14,
	  &LATA, &TRISA, &PORTA, 1 << 4, 2 },
	{ &LATB, 1 << 7, &RPB7R, &LATB, 1 << 8,
	  &LATB, &TRISB, &PORTB, 1 << 5, -1 },
	{ &LATB, 1 << 15, &RPB15R, &LATB, 1 << 9,
	  &LATB, &TRISB, &PORTB, 1 << 13, 3 },
};

#define U2RXR_RPB5 1

static W rxtarget = 0;	/* serial -> USB, and HEX is written here */
static W txmask = 1;	/* USB serial -> these targets */
static const struct target_pins *cur = &targets[0];

#define LAT_PGC(v) (cur->pgclat[(v) ? 2 : 1] = cur->pgcbit)
#define LAT_MCLR(v) (cur->mclrlat[(v) ? 2 : 1] = cur->mclrbit)
#define LAT_PGD(v) (cur->pgdlat[(v) ? 2 : 1] = cur->pgdbit)
#define TRIS_PGD(v) (cur->pgdtris[(v) ? 2 : 1] = cur->pgdbit)
#define PORT_PGD ((cur->pgdport[0] & cur->pgdbit) ? 1 : 0)
...
static void idletask(void)
{
	...
	/* UART1/UART2 RX (only the selected target is routed) */
	if (targets[rxtarget].rxsel >= 0) {
		if ((U1STAbits.OERR))
			U1STA = 0x1400;
...
/*
 * Route UTX1 to the PGC pins in txmask and the selected target's PGD
 * to U1RX (or U2RX for target 2).  Unselected PGC pins stay GPIO high
 * (UART idle level).
 */
static void route_serial(void)
{
	W i;

	for (i = 0; i < NTARGET; i++) {
		targets[i].pgclat[2] = targets[i].pgcbit;
		*targets[i].pgcrp = (txmask & (1 << i)) ? 1 : 0; /* UTX1 : i/o */
	}
	if (targets[rxtarget].rxsel >= 0) {
		U1RXR = targets[rxtarget].rxsel;
		U2RXR = 0; /* dummy:RA1 */
	} else {
		U1RXR = 0; /* dummy:RA2 */
		U2RXR = U2RXR_RPB5;
...
/* Pulse MCLR of all targets. */
static void reset_all(void)
{
	W i;

	for (i = 0; i < NTARGET; i++)
		targets[i].mclrlat[1] = targets[i].mclrbit;
	wait1ms();
	for (i = 0; i < NTARGET; i++)
		targets[i].mclrlat[2] = targets[i].mclrbit;
}
...
static void rsproc(void)
{
	...
	if ((linewpos == 0) && (c == 0xfe)) {
		/* Custom ":feXY" target select; XY follows. */
		linewpos = 0x7fff;
		return;
	}
	if (linewpos == 0x7fff) {
		/*
		 * Refused while a programming session is open (writing) or
		 * HEX data is pending in writebuf, since that data belongs
		 * to the currently selected target.
		 */
		if (((c >> 4) < NTARGET) && !writing && (writebufwsize == 0)) {
			rxtarget = c >> 4;
			txmask = c & 0xf;
			cur = &targets[rxtarget];
			route_serial();
			p2ustr("sel ");
			p2uub(c);
			p2ustr("\r\n");
...
void main(void)
{
	...
	TRISA = 0x0010; /* -------- ---I--OO */
	TRISB = 0x2826; /* OOI-I-OO OOIOOIIO */
	...
	U2MODE = 0;
	U2BRG = 86; /* 115.4kbps, rx only (target 2) */
	U2MODE = 0x8008;
	U2STA = 0x1000; /* rx-enable */
