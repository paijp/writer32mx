 ...
 *   Local debug log:  UTX2 mirror on RPB0 (P4) - RB10/RB11 belong to
 *     USB here; muted once the writer loop starts because UART2 then
...
/* ============================================================ */
/* Local debug log (UTX2 mirror on RPB0)                        */
/*                                                              */
...
static	void	app_init(void)
{
	...
	/* Target-facing pins: MCLR (RA1) held low until "run"; PGC (RB9)
	   idle high; PGD (RB8) input.  Other pins keep their POR state. */
	LATAbits.LATA1 = 0;
	TRISAbits.TRISA1 = 0;
	LATBbits.LATB9 = 1;
	TRISBbits.TRISB9 = 0;
	...
	/* UART2: local debug mirror on RPB0 (P4) during setup; routed onto
	   the target pins (RPB9/RPB8) once the writer loop starts. */
	RPB0R = 2;		/* UTX2 */
#ifdef DEBUG_UART_SCAN
